...
public class Sen_BSEC2Get extends TranslatorBlock
{
  ...
  public String toCode() throws SocketNullException, SubroutineNotDeclaredException
  {
	...
	Helper = "void newDataCallback(const bme68xData data, const bsecOutputs outputs, Bsec2 bsec)\r\n"
			...
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tIAQ = \" + String(output.signal));\r\n"
			+ "      iaqSensor.iaq=output.signal;\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tIAQ accuracy = \" + String((int) output.accuracy));\r\n"
			+ "      iaqSensor.iaqAccuracy = output.accuracy;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_RAW_TEMPERATURE:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tTemperature = \" + String(output.signal));\r\n"
			+ "      iaqSensor.rawTemperature = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_RAW_PRESSURE:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tPressure = \" + String(output.signal));\r\n"
			+ "      iaqSensor.pressure = output.signal*100.;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_RAW_HUMIDITY:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tHumidity = \" + String(output.signal));\r\n"
			+ "      iaqSensor.rawHumidity = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_RAW_GAS:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tGas resistance = \" + String(output.signal));\r\n"
			+ "      iaqSensor.gasResistance=output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_STABILIZATION_STATUS:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tStabilization status = \" + String(output.signal));\r\n"
			+ "      iaqSensor.stabilizationStatus = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_RUN_IN_STATUS:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tRun in status = \" + String(output.signal));\r\n"
			+ "      iaqSensor.runInStatus = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_TEMPERATURE:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tCompensated temperature = \" + String(output.signal));\r\n"
			+ "      iaqSensor.temperature = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_HUMIDITY:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tCompensated humidity = \" + String(output.signal));\r\n"
			+ "      iaqSensor.humidity = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_STATIC_IAQ:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tStatic IAQ = \" + String(output.signal));\r\n"
			...
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tbVOC equivalent = \" + String(output.signal));\r\n"
			+ "      iaqSensor.breathVocEquivalent = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_GAS_PERCENTAGE:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tGas percentage = \" + String(output.signal));\r\n"
			+ "      iaqSensor.gasPercentage = output.signal;\r\n"
			+ "      break;\r\n"
			+ "    case BSEC_OUTPUT_COMPENSATED_GAS:\r\n"
			+ "      if (iaqSensor.debug) IOTW_PRINTLN(\"\\tCompensated gas = \" + String(output.signal));\r\n"
