...
//				+ "  IOTW_PRINTLN(\"\\ncallback topic:\" + topic + \", payload:\" + MQTT_Rx_Payload);\n"
		...
		translator.addDefinitionCommand(def);
	     ...
	     def =  "//------------ reconnect mqtt-client\n"
				...
				+ "   IOTW_PRINT(\"Attempting MQTT connection...\");\n"
				...
				+ "       IOTW_PRINT(\"subscribe on topic: \");IOTW_PRINTLN(mqtt_sub[i].topic);\n"
				+ "     }\n"
				+"    } else { \n"
				+"      IOTW_PRINT(\"failed: \");\n"
				+"      IOTW_PRINT(translateMqttError(mqttclient.state()));\n"
				+"      IOTW_PRINTLN(\" try again in 5 seconds\");\n"
				+"      delay(5000);\n"
				+"   }\n"
				+"  }\n"
				+" } else { \n"
